...
func (srv *Server) run() {
	...
	if len(srv.Config.Protocols) > 0 {
		srv.log.Warn("Started P2P networking", "version", srv.Config.Protocols[0].Version, "self", srv.localnode.Node().URLv4(), "name", srv.Name)
	}
	defer srv.loopWG.Done()
...
func (srv *Server) postHandshakeChecks(peers map[enode.ID]*Peer, inboundCount int, c *conn) error {
	...
	case !c.is(trustedConn) && len(peers) >= srv.MaxPeers:
		log.Warn("disc1", "name", c.name)
		return DiscTooManyPeers
	case !c.is(trustedConn) && c.is(inboundConn) && inboundCount >= srv.maxInboundConns():
		log.Warn("disc2", "name", c.name)
		return DiscTooManyPeers
	case peers[c.node.ID()] != nil:
		log.Warn("disc3", "name", c.name)
		return DiscAlreadyConnected
	case c.node.ID() == srv.localnode.ID():
		log.Warn("disc4", "name", c.name)
		return DiscSelf
	default:
		return nil
	}
